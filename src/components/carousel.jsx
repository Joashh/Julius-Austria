...
export default function Carousel() {
  ...
  const textToCopy = "https://juliusasheraustria.vercel.app/#";
  ...
  try {
    if (navigator.clipboard && window.isSecureContext) {
      ...
      // ⚙️ Fallback for older browsers or HTTP
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "ArrowLeft") {
        handlePageChange(currentCert - 1 === 0 ? 16 : currentCert - 1);
      } else if (event.key === "ArrowRight") {
        handlePageChange((currentCert % 16) + 1);
      }
    };
      ...
      <div className="relative flex dark:bg-gray-900 bg-gray-200 justify-center items-center w-full h-[60vh] rounded-xl overflow-hidden carousel-height">
        ...
        <div className="absolute bottom-0 right-0 items-center left-0 h-full p-3 flex justify-between w-full">
          <FaAngleDoubleLeft
            ...
            onClick={() => handlePageChange(currentCert - 1 === 0 ? 16 : currentCert - 1)}
          />
          <FaAngleDoubleRight
            className="h-5 w-5 cursor-pointer text-black dark:text-white"
            style={{ filter: 'drop-shadow(0px 2px 4px rgba(0, 0, 0, 0.6))' }}
            onClick={() => handlePageChange((currentCert % 16) + 1)}
          />
        </div>
